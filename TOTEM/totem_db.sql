-- phpMyAdmin SQL Dump
-- Versão para Localhost (Com Trigger Automático)
--
...
--
-- Criação do banco de dados local (caso não exista)
--
CREATE DATABASE IF NOT EXISTS `totem_db` DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
USE `totem_db`;
...
--
-- Criamos a tabela de pedidos primeiro, pois a trigger de 'itens_pedido' precisa dela para buscar os nomes.
...
--
-- Dados extraídos da tabela `pedidos`
--
...
--
-- Dados extraídos da tabela `itens_pedido`
--
...
(36, 13, 'Carpaccio di Manzo', '46.00', 1, 'Weslley'), -- Sincronizado do pedido 13
...
--
-- Índices para tabelas duplicadas
--
...
--
-- Índices de `itens_pedido`
--
ALTER TABLE `itens_pedido`
  ADD PRIMARY KEY (`id`);

--
-- AUTO_INCREMENT para tabelas duplicadas
--

--
-- AUTO_INCREMENT de `pedidos`
--
ALTER TABLE `pedidos`
  MODIFY `id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=20;

--
-- AUTO_INCREMENT de `itens_pedido`
--
ALTER TABLE `itens_pedido`
...
--
-- Gatilho (Trigger) para Autopreencher o nome do cliente
--
DROP TRIGGER IF EXISTS `tg_preencher_cliente_nome`;
DELIMITER //

CREATE TRIGGER `tg_preencher_cliente_nome` 
BEFORE INSERT ON `itens_pedido` 
FOR EACH ROW 
BEGIN
    -- Busca automaticamente o 'cliente_nome' na tabela 'pedidos' com base no 'pedido_id' que está sendo inserido
    SET NEW.cliente_nome = (SELECT `cliente_nome` FROM `pedidos` WHERE `id` = NEW.pedido_id);
END;
//
